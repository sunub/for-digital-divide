...
import pg from 'pg';
import { date, maxLength, maxValue, minLength, minValue, object, safeParse, string } from 'valibot';
...
const AccountSchema = object({
  user_id: string('사용자 아이디', [maxLength(255, '255자리의 계좌번호를 입력해주세요.')]),
  account_number: string('계좌번호', [
    minLength(14, '14자리의 계좌번호를 입력해주세요.'),
    maxLength(14, '14자리의 계좌번호를 입력해주세요.'),
  ]),
  created_date: date([
    minValue(new Date(2021, 0, 1), '2021년 1월 1일 이후의 날짜를 입력해주세요.'),
    ...
    if (checkAccount.success) {
      Object.entries(checkAccount.output).forEach(([key, value]) => {
        const accountKey = key
          .split('_')
          .map((word, i) => (i === 1 ? word[0].toUpperCase() + word.slice(1) : word))
          .join('');
