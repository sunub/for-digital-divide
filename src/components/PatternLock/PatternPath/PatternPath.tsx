...
const getAngle = (x1: number, x2: number, y1: number, y2: number) => (Math.atan2(y2 - y1, x2 - x1) * 180) / Math.PI;
...
function PatternPath(props: PatternPathProps) {
  ...
  const getLineData = React.useCallback(
    (start: number, end: number | { x: number; y: number }, width: number) => {
      const x1 = (start % 3) * (width / 3) + width / 6;
      const y1 = Math.floor(start / 3) * (width / 3) + width / 6;
      let x2, y2;
      if (typeof end === 'number') {
        x2 = (end % 3) * (width / 3) + width / 6;
        y2 = Math.floor(end / 3) * (width / 3) + width / 6;
  ...
  React.useEffect(() => {
    ...
    if (path.length >= 2) {
      ...
      for (let i = 0; i < l; i++) {
        const lineData = getLineData(path[i], path[i + 1], width);
