...
import { TransactionSchema, TransactionIdSchema, AccountNumberSchema } from "./transaction.model";
...
export async function getAllTransactionsController() {
  try {
    ...
    return NextResponse.json(parsedTransactions.map(result => result.data), { status: 200 });
  } catch (error) {
