...
export async function transactionTypesController(request: Request) {
  ...
  if (!parsedTypeCode.success) {
    return NextResponse.json({ error: 'Invalid transaction type code format' }, { status: 400 });
  }
  ...
  try {
    ...
    if (!parsedTransactionType.success) {
      return NextResponse.json({ error: 'Invalid transaction type data format' }, { status: 500 });
    }
