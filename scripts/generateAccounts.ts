...
function createAccount(user_id: number): Account {
  const account_type: AccountCode = ACCOUNT_TYPES[Math.floor(Math.random() * ACCOUNT_TYPES.length)];
  const balance =
    account_type === 'CREDIT' ? -Math.floor(Math.random() * 1000000) : Math.floor(Math.random() * 10000000) + 100000;
  ...
  return {
    ...
    account_type,
    balance,
    created_at: new Date(Date.now() - Math.floor(Math.random() * 2 * 365 * 24 * 60 * 60 * 1000)),
  };
}
