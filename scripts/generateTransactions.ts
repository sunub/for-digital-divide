 ...
 */
import { Transaction } from '@/entities/transactions/transaction.model';
...
const TRANSACTION_INFO: Record<TransactionCode, string[]> = {
  DEPOSIT: ['월급 입금', '보너스 입금', '자금 이체', '부업 수입', '배당금 입금', '카드대금 납부'],
...
const getAmountRange = (transactionType: TransactionCode): { min: number; max: number } => {
...
const getRandomTransactionType = (accountType: AccountCode): TransactionCode => {
  const allowedTypes = ALLOWED_TRANSACTIONS[accountType] || [];
  if (allowedTypes.length === 0) return 'DEPOSIT';

  const weights: Record<AccountCode, Partial<Record<TransactionCode, number>>> = {
  ...
  for (const type in accountWeights) {
    const weight = accountWeights[type as TransactionCode]!;
    cumulativeWeight += weight;
    if (random <= cumulativeWeight && allowedTypes.includes(type as TransactionCode)) {
...
export async function generateTransactions(accounts: Account[]): Promise<GenerationResult> {
  console.log(`\n▶️  실행 환경: ${isProduction ? 'Production' : 'Development'}`);
  console.log(`Transactions generation started for ${accounts.length} accounts`);
  ...
  const accountBalances = new Map<number, number>(accounts.map((acc) => [acc.account_number, acc.balance]));
  ...
  for (let i = 0; i < accounts.length; i++) {
    ...
    const transactionCount = totalTransactions + (i < extraTransactions ? 1 : 0);

    for (let j = 0; j < transactionCount; j++) {
      const transaction_type = getRandomTransactionType(account_type as AccountCode);
      const currentBalance = accountBalances.get(account_number)!;
      let amount = 0;
      ...
      if (transaction_type === 'WITHDRAWAL' || transaction_type === 'PAYMENT') {
        ...
        const maxWithdrawal = account_type === 'CREDIT' ? max : Math.min(max, currentBalance - 1000);
      ...
      const occurred_at = new Date(startDate.getTime() + Math.random() * (now.getTime() - startDate.getTime()));
      amount = Math.floor(
        amount * getMonthlyMultiplier(occurred_at.getMonth()) * getDayOfWeekMultiplier(occurred_at.getDay()),
      );
  ...
  transactions.sort((a, b) => a.occurred_at.getTime() - b.occurred_at.getTime());
