...
describe('TransferService Integration Tests', () => {
  beforeAll(async () => {
    ...
    const result = await prisma.$queryRaw<[{ current_schema: string }]>`SELECT current_schema()`;
    const currentSchema = result[0]?.current_schema;
    
    if (currentSchema !== 'test_db') {
      throw new Error(`CRITICAL: Test is running against schema '${currentSchema}', not 'test_db'! Aborting to prevent data loss.`);
    }
  });
  ...
  beforeEach(async () => {
    ...
    await prisma.accounts.create({
      data: { account_number: BigInt(1001), user_id: userA.user_id, balance: 100000 },
    });
    await prisma.accounts.create({
      data: { account_number: BigInt(1002), user_id: userB.user_id, balance: 0 },
    });
  });
  ...
  it('Tracer Bullet: 정상적인 송금 성공 시 두 계좌의 잔액이 변경되고 거래내역이 남는다', async () => {
    ...
    // Execute
    await transferService.executeTransfer(fromAccount, toAccount, transferAmount);

    // Assert (검증)
    const senderAccount = await prisma.accounts.findUnique({ where: { account_number: fromAccount } });
    const receiverAccount = await prisma.accounts.findUnique({ where: { account_number: toAccount } });
