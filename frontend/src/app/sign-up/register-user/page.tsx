import { Grid, Flex, Text } from "@for-digital-divide/design-system";
...
function RegisterGuide({ hasSession }: { hasSession: boolean }) {
      ...
      <Instruction.Panel>
        ...
        >
          <Text as="p" variant="body" color={"text"}>
            현재 계정으로 로그인된 상태입니다. 대시보드로 이동하여 간편 송금 및 자산 관리를 즉시 시작하실 수 있습니다. 혹시 새로운 다른 계정으로 가입하시려면 아래 로그아웃을 이용해 주세요.
          </Text>
        </Instruction.InfoBox>
      </Instruction.Panel>
    );
  ...
  return (
    <Instruction.Panel>
      <Instruction.Badge
        icon={
          <MdBadge size={18} style={{ color: "var(--color-button)" }} />
        }
      >
        쉬운 회원 가입
      </Instruction.Badge>
      ...
      >
        <Text as="p" variant="body" color={"text"}>
          사용자 본인의 실명(사용자 이름)을 올바르게 입력해 주세요. 이메일 주소와 영문 대/소문자, 숫자, 특수문자가 모두 들어간 안전한 비밀번호를 사용하여 계정을 보호하세요.
        </Text>
      </Instruction.InfoBox>
    </Instruction.Panel>
  );
        ...
        <Device.Content>
          {hasSession ? (
            ...
            <Flex
              ...
              gap="2rem"
              style={{ padding: "24px", paddingTop: "48px", boxSizing: "border-box" }}
            >
              <h1 className={style.phoneTitle} style={{ textAlign: "center", width: "100%" }}>회원 가입</h1>
